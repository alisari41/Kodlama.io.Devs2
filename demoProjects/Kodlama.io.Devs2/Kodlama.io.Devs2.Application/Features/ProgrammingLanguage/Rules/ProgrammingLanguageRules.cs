...
namespace Kodlama.io.Devs2.Application.Features.ProgrammingLanguage.Rules
{
    public class ProgrammingLanguageRules
    {
        ...
        public void ProgrammingLanguageIDShouldExitsWhenRequested(Domain.Entities.ProgrammingLanguage programmingLanguage)
        {
            // Eğer bir Programlama Dili  talep ediliyorsa o dilin olması gerekir.
            if (programmingLanguage == null) throw new BusinessException("Programalama dili mevcut değildir."); // BusinessException için "Core.CrossCuttingConcerns" dan Referans almak gerekir
        }

        public async Task ProgralamaDiliYok(int id)
        {
            // Metod geriye hiç bir şey dönmüyor. Hiçbir şey dönmüyorsa geçiyor demektir. Kuralldan geçmediği her durumda hata fırlatıyor demektir.
            // Tablo Adı ( Programlama Dilleri ) isimleri tekrar edemez
            IPaginate<Domain.Entities.ProgrammingLanguage> result = await _programmingLanguageRepository.GetListAsync(x => x.Id == id);// IPaginate sayfalama yapmak için yazıldı
            if (!result.Items.Any()) throw new BusinessException("Progralama Dili bulumanadı!"); // BusinessException için "Core.CrossCuttingConcerns" dan Referans almak gerekir
        }


        public async Task ProgrammingIdCanNotBeDuplicatedWhenInserted(int id)
        {
            // Metod geriye hiç bir şey dönmüyor. Hiçbir şey dönmüyorsa geçiyor demektir. Kuralldan geçmediği her durumda hata fırlatıyor demektir.
            // Tablo Adı ( Program Dil, ) isimleri tekrar edemez
            IPaginate<Domain.Entities.ProgrammingLanguage> result = await _programmingLanguageRepository.GetListAsync(b => b.Id == id); // IPaginate sayfalama yapmak için yazıldı
            if (!result.Items.Any()) throw new BusinessException("Progralama Dili bulumanadı!"); // BusinessException için "Core.CrossCuttingConcerns" dan Referans almak gerekir
        }
    }
}
